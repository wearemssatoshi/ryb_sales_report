...
def generate_2024_report():
    # Step 1: Define the source CSV path
    csv_path = '/Users/satoshiiga/dotfiles/KRMSALES/2024/2024_sales_report.csv'
    html_path = '/Users/satoshiiga/dotfiles/KRMSALES/2024/2024_sales_report.html'

    # Step 2: Read and sanitize the CSV data
    all_rows = []
    try:
        with open(csv_path, 'r', encoding='utf-8') as f:
            reader = csv.DictReader(f)
            for row in reader:
                # Sanitize date format (e.g., "1_18" -> "1/18")
                date_str = row.get("Date", "").replace("_", "/")
                row["Date"] = date_str
                all_rows.append(row)
    ...
    # Step 3: Calculate all necessary values
    report_data = {}
    grand_total_sales = sum(int(r["Sales"]) for r in all_rows if r.get("Sales"))
    fiscal_year_months = [(2024, m) for m in range(4, 13)] + [(2025, m) for m in range(1, 4)]
    day_order = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"]

    for year, month in fiscal_year_months:
        month_key = f"{year}-{month:02d}"
        
        # Filter rows for the current month, handling inconsistent date formats
        monthly_rows = [r for r in all_rows if r.get("Date") and r["Date"].startswith(f"{month}/")]
    ...
    # Step 4: Generate the HTML string
    html = """<!DOCTYPE html>
<html lang="ja">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>2024年度 売上レポート</title>
    <style>
...
<body>
    <h1>2024年度 売上レポート (2024年4月 - 2025年3月)</h1>
"""
...
"""
        for row in month_data['daily_rows']:
            try:
                m_str, d_str = row["Date"].split("/")
                sales_formatted = f"¥{int(row['Sales']):,}"
                html += f"                        <tr><td>{m_str}/{d_str}</td><td>{row['Day']}</td><td>{sales_formatted}</td></tr>\n"
...
"""
    ...
    <div class="grand-total">
        2024年度 年間総売上: {grand_total_formatted}
    </div>
</body>
</html>"""

    # Step 5: Write the generated HTML to the file
    try:
        with open(html_path, "w", encoding="utf-8") as f:
            f.write(html)
        print(f"Report '{html_path}' regenerated successfully.")
    except IOError as e:
        print(f"Error writing to file: {e}")

if __name__ == "__main__":
    generate_2024_report()
